 ...
 */
export const dailyBackup = functionsV2.scheduler.onSchedule('0 2 * * *', async (event) => {
 ...
 */
export const cleanupOldData = functionsV2.scheduler.onSchedule({
  schedule: '0 3 * * 0',
 ...
 */
export const syncExternalData = functions.pubsub.schedule('0 4 * * *')
  .timeZone('America/New_York')
  .onRun(async (context) => {
    try {
 ...
 */
export const createManualBackup = functionsV2.https.onCall(async (request) => {
  ...
  try {
    ...
    throw new functionsV2.https.HttpsError('internal', `Backup failed: ${(error as Error).message}`);
  }
});
