...
exports.userManagement = {
    ...
    updateUserProfile: functionsV2.https.onCall(async (request) => {
        ...
        if (!request.auth) {
            throw new functionsV2.https.HttpsError('unauthenticated', 'You must be logged in to update your profile.');
        }
        const userId = request.auth.uid;
        const { data } = request;
        const { updates } = data;
        if (!updates || typeof updates !== 'object') {
            throw new functionsV2.https.HttpsError('invalid-argument', 'Updates object is required.');
        }
        try {
            ...
            throw new functionsV2.https.HttpsError('internal', `Error updating profile: ${error.message}`);
        }
    }),
    /**
    ...
    getUserAnalytics: functionsV2.https.onCall(async (request) => {
        // Ensure user is authenticated
        if (!request.auth) {
            throw new functionsV2.https.HttpsError('unauthenticated', 'You must be logged in to view analytics.');
        }
        const userId = request.auth.uid;
        const { data } = request;
        ...
        if (!isAdmin && requestedUserId !== userId) {
            throw new functionsV2.https.HttpsError('permission-denied', 'You can only view your own analytics.');
        }
        try {
            // Get user profile
            ...
            throw new functionsV2.https.HttpsError('internal', `Error retrieving analytics: ${error.message}`);
        }
    }),
    /**
    ...
    trackUserLogin: functionsV2.https.onCall(async (request) => {
        ...
        try {
            ...
            throw new functionsV2.https.HttpsError('internal', `Error tracking login: ${error.message}`);
        }
    }),
};
