...
exports.notificationService = {
    ...
    sendOrderNotification: functionsV2.https.onCall(async (request) => {
        if (!request.auth) {
            throw new functionsV2.https.HttpsError('unauthenticated', 'You must be logged in to send notifications.');
        }
        const { data } = request;
        const { orderId, type, customMessage } = data;
        if (!orderId || !type) {
            throw new functionsV2.https.HttpsError('invalid-argument', 'Order ID and notification type are required.');
        }
        try {
            // Get order data
            const orderDoc = await db.collection('orders').doc(orderId).get();
            if (!orderDoc.exists) {
                throw new functionsV2.https.HttpsError('not-found', 'Order not found.');
            }
            const orderData = orderDoc.data();
            if (!orderData) {
                throw new functionsV2.https.HttpsError('internal', 'Order data is missing.');
            }
            // Get user data
            ...
            if (!userData) {
                throw new functionsV2.https.HttpsError('internal', 'User data is missing.');
            }
          ...
          `;
                ...
                case 'custom':
                    ...
                    break;
                default:
                    throw new functionsV2.https.HttpsError('invalid-argument', 'Invalid notification type.');
            }
            const results = [];
            ...
            throw new functionsV2.https.HttpsError('internal', `Failed to send order notification: ${error.message}`);
        }
    }),
    /**
    ...
    sendWelcomeEmail: functionsV2.https.onCall(async (request) => {
        if (!request.auth) {
            throw new functionsV2.https.HttpsError('unauthenticated', 'You must be logged in to send welcome email.');
        }
        ...
        try {
            ...
            const userData = userDoc.data();
            if (!userData) {
                throw new functionsV2.https.HttpsError('not-found', 'User data not found.');
            }
            if (!userData.email) {
                throw new functionsV2.https.HttpsError('invalid-argument', 'User email not found.');
            }
            const transporter = createTransporter();
            const mailOptions = {
                from: functions.config().email?.from || process.env.EMAIL_FROM || 'noreply@yourapp.com',
        ...
        `,
            ...
            throw new functionsV2.https.HttpsError('internal', `Failed to send welcome email: ${error.message}`);
        }
    }),
    /**
    ...
    sendPasswordResetEmail: functionsV2.https.onCall(async (request) => {
        ...
        `,
            ...
            throw new functionsV2.https.HttpsError('internal', `Failed to send password reset email: ${error.message}`);
        }
    }),
    /**
    ...
    sendPromotionalEmail: functionsV2.https.onCall(async (request) => {
        ...
        if (!request.auth) {
            throw new functionsV2.https.HttpsError('unauthenticated', 'You must be logged in to send promotional emails.');
        }
        const { data } = request;
        const { subject, htmlContent, targetUsers, sendToAll } = data;
        if (!subject || !htmlContent) {
            throw new functionsV2.https.HttpsError('invalid-argument', 'Email and subject are required.');
        }
        try {
            ...
            throw new functionsV2.https.HttpsError('internal', `Failed to send promotional email: ${error.message}`);
        }
    }),
    /**
    ...
    sendPushNotification: functionsV2.https.onCall(async (request) => {
        ...
        if (!title || !body) {
            throw new functionsV2.https.HttpsError('invalid-argument', 'User ID, title, and body are required.');
        }
        try {
            const results = [];
            if (topic) {
                ...
                const userDocs = await Promise.all(targetUsers.map((userId) => db.collection('users').doc(userId).get()));
                const tokens = [];
                ...
                throw new functionsV2.https.HttpsError('invalid-argument', 'Either topic or targetUsers must be provided.');
            }
            return { success: true, results };
        }
        catch (error) {
            console.error('Error sending push notification:', error);
            throw new functionsV2.https.HttpsError('internal', `Failed to send push notification: ${error.message}`);
        }
    }),
};
