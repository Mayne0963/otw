...
export const updateServiceStatus = async (serviceId: string, status: string) => {
...
export const getTierMembership = async (userId: string) => {
  ...
  const q = query(collection(db, "tier_memberships"), where("userId", "==", userId))
...
export const createTierMembership = async (membershipData: any) => {
  const { collection, addDoc } = await import("firebase/firestore")
  const docRef = await addDoc(collection(db, "tier_memberships"), membershipData)
...
export const updateVolunteerHours = async (volunteerId: string, hours: number) => {
  const { doc, updateDoc, getDoc } = await import("firebase/firestore")
