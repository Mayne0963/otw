    ...
    const res = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: 'user', content: `Rewrite this menu item description to be enticing and clear: ${desc}` },
        ],
        max_tokens: 60,
      }),
    })
...
async function scrapeMenu(url: string, source: 'broskis' | 'partner'): Promise<MenuItem[]> {
  try {
    ...
    $('.menu-item').each((_, el) => {
      try {
        ...
        const priceRaw = $(el).find('.menu-item-price').text().replace(/[^\d.]/g, '')
