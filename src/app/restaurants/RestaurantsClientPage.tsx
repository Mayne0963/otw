...
export default function RestaurantsClientPage() {
  ...
  useEffect(() => {
    ...
    if (filters.dietaryOptions.length > 0) {
      filtered = filtered.filter(
        (restaurant) =>
          restaurant.dietaryOptions !== undefined &&
          filters.dietaryOptions.some((option) =>
            restaurant.dietaryOptions?.includes(option) || false
          )
      );
    }
    ...
    if (filters.deliveryTimeMax !== null) {
      filtered = filtered.filter((restaurant) => {
        if (!restaurant.deliveryTime) return false;
        // Use optional chaining to safely access deliveryTime
        const parts = restaurant.deliveryTime?.split("-");
        if (!parts || parts.length < 2) return false;
        const maxTime = Number.parseInt(parts[1] || "0");
        if (isNaN(maxTime)) return false;
    ...
    if (filters.distance !== null) {
      filtered = filtered.filter(
        (restaurant) => restaurant.distance <= (filters.distance || 0),
      );
    }
    ...
    if (filters.features.length > 0) {
      filtered = filtered.filter(
        (restaurant) =>
          restaurant.features !== undefined &&
          filters.features.some((feature) =>
            restaurant.features?.includes(feature) || false
          )
      );
    }
    ...
    switch (filters.sortBy) {
      ...
      case "deliveryTime":
        filtered.sort((a, b) => {
          ...
          const aTime = Number.parseInt(a.deliveryTime?.split("-")[0] || "0");
          const bTime = Number.parseInt(b.deliveryTime?.split("-")[0] || "0");
          return aTime - bTime;
        });
        break;
      case "deliveryFee":
